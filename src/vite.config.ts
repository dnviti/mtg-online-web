...
export default defineConfig(({ mode }) => {
  // Load env file from parent directory (src/) where .env is located
  const env = loadEnv(mode, path.resolve(__dirname), '');

  return {
    define: {
      // Expose DEV_MODE environment variable to client
      'import.meta.env.VITE_DEV_MODE': JSON.stringify(env.DEV_MODE || 'false')
    },
    plugins: [
      react(),
      VitePWA({
