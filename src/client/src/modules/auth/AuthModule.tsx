...
export const AuthModule: React.FC<{ onSuccess: () => void }> = ({ onSuccess }) => {
    ...
    useEffect(() => {
        const checkGoogleOAuth = async () => {
            try {
                const response = await ApiService.get<{ available: boolean }>('/api/auth/google/status');
                setGoogleAvailable(response.available);
            } catch {
            ...
            {googleAvailable && (
                <div className="w-full mt-6">
                    ...
                    <button
                        ...
                        className="mt-4 w-full flex items-center justify-center gap-3 py-3 px-4 border border-slate-600 rounded-lg bg-slate-900 hover:bg-slate-700 transition-all"
                    >
                        <GoogleIcon className="w-5 h-5" />
                        <span className="text-slate-300 font-medium">Continua con Google</span>
                    </button>
                </div>
            )}
